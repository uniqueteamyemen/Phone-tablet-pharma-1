 ...
 * Deliberately removes all punctuation (dots, commas, hyphens, slashes), collapses spaces,
 * and harmonizes Arabic characters (Alef, Yaa, Taa Marbuta, diacritics).
 */
export function normalizeMedicineSearch(value: string | null | undefined): string {
  return String(value ?? "")
    ...
    .replace(/[أإآٱ]/g, "ا")
    .replace(/\u0649/g, "ي")
    .replace(/ة/g, "ه") // Taa Marbuta normalization
...
function compactSearchValue(value: string): string {
  return normalizeMedicineSearch(value).replace(/\s+/g, "");
}
...
function fuzzyScore(queryName: string, candidateName: string): number {
  ...
  for (const value of candidates) {
    ...
    if (Math.abs(query.length - value.length) > allowance) continue;
    
    // Quick first-letter heuristic: if both strings have >= 5 chars and first chars differ, skip
    if (query.length >= 5 && value.length >= 5 && query[0] !== value[0] && query[1] !== value[0] && query[0] !== value[1]) {
      continue;
    }
...
const recordNameCache = new WeakMap<MedicineSearchRecord, string[]>();
const recordContextCache = new WeakMap<MedicineSearchRecord, string[]>();
const recordStrengthCache = new WeakMap<MedicineSearchRecord, string[]>();

function cachedMedicineNameValues(record: MedicineSearchRecord): string[] {
  let cached = recordNameCache.get(record);
  if (!cached) {
    cached = medicineNameValues(record);
    recordNameCache.set(record, cached);
  }
  return cached;
}

function cachedMedicineContextValues(record: MedicineSearchRecord): string[] {
  let cached = recordContextCache.get(record);
  if (!cached) {
    cached = medicineContextValues(record);
    recordContextCache.set(record, cached);
  }
  return cached;
}

function cachedMedicineStrengthValues(record: MedicineSearchRecord): string[] {
  let cached = recordStrengthCache.get(record);
  if (!cached) {
    cached = medicineStrengthValues(record);
    recordStrengthCache.set(record, cached);
  }
  return cached;
}
 ...
 */
export function getMedicineSearchMatch(
  record: MedicineSearchRecord, 
  query: string,
  precomputedQuery?: { normalizedQuery: string; queryName: string; queryStrengths: string[] }
): MedicineSearchMatch {
  const normalizedQuery = precomputedQuery?.normalizedQuery ?? normalizeMedicineSearch(query);
  const queryName = precomputedQuery?.queryName ?? nameWithoutStrength(query);
  if (!normalizedQuery || !queryName) return { score: 0, kind: "none", strengthMatched: false };

  const queryStrengths = precomputedQuery?.queryStrengths ?? strengthValues(query);
  const recordStrengths = cachedMedicineStrengthValues(record);
  ...
  for (const field of cachedMedicineNameValues(record)) {
  ...
  if (!score && cachedMedicineContextValues(record).some((field) => field.includes(normalizedQuery))) {
    score = 400;
    kind = "contains";
  }
...
export function medicineSearchFields(record: MedicineSearchRecord): string {
  return [...cachedMedicineNameValues(record), ...cachedMedicineContextValues(record)].join(" ");
}
...
export function rankMedicinesBySearch<T extends MedicineSearchRecord>(records: T[], query: string): T[] {
  ...
  if (!normalizedQuery || !queryName) return [];

  const queryStrengths = strengthValues(query);
  const precomputed = { normalizedQuery, queryName, queryStrengths };

  const matched: { record: T; index: number; match: MedicineSearchMatch }[] = [];

  for (let index = 0; index < records.length; index++) {
    const record = records[index];
    const match = getMedicineSearchMatch(record, query, precomputed);
    if (match.score > 0) {
      matched.push({ record, index, match });
    }
  }
